...
import { calculateGST, generateQuoteNumber } from '@/lib/utils'
...
function QuoteForm() {
  ...
  const addMaterial = () => {
    setMaterials([
      ...materials,
      { id: String(Date.now()), name: '', quantity: 1, unit_cost: 0, total: 0 },
    ])
  }
        ...
        if (data.materials) {
          setMaterials(
            data.materials.map((mat: any, i: number) => ({
              ...
              total: (mat.quantity || 1) * (mat.unit_cost || 0),
            }))
          )
        }
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <Card>
        ...
        {materials.length === 0 ? (
          <p className="py-4 text-center text-sm text-muted">No materials added yet</p>
        ) : (
          <div className="space-y-3">
            {materials.map((mat) => (
              <div
              ...
              >
                ...
                <div className="w-24">
                  <label className="mb-1 text-xs text-muted">Unit Cost ($)</label>
                  <input
                    type="number"
                    min="0"
                    step="0.01"
                    value={mat.unit_cost}
                    onChange={(e) => updateMaterial(mat.id, 'unit_cost', parseFloat(e.target.value) || 0)}
                    className="w-full rounded-md border border-border bg-background px-2.5 py-1.5 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-primary/50"
                  />
                </div>
                <div className="w-24">
      ...
      <Card>
        <div className="space-y-2 border-b border-border pb-4">
          <div className="flex justify-between text-sm">
            ...
            <span className="text-foreground">${laborSubtotal.toFixed(2)}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-muted">Materials</span>
            <span className="text-foreground">${materialSubtotal.toFixed(2)}</span>
          </div>
          <div className="flex justify-between text-sm">
      ...
      <Card>
        <div className="space-y-4">
          ...
          <div>
            <label className="mb-1.5 block text-sm font-medium text-muted">Terms & Conditions</label>
            <textarea
              value={terms}
